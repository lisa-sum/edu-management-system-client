...
export default function Room() {
  const messageList = useAppSelector(
    (state: RootState) => state.message.value.messageList,
  )
  const [chatBarList] = useState<{text: string}[]>([
    {text: '聊天'},
    {text: '简介'},
  ])

  const {number} = useAppSelector((state: RootState) => state.room.value)
  const {account, avatar} = useAppSelector(
